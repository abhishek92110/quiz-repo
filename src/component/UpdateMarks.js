...
const UpdateMarks = () => {
  ...
  const downloadPDF = () => {
    ...
    let yOffset = 10; // Vertical offset for each item

    doc.setFontSize(16);
    doc.text("Quiz Results", 10, yOffset);
    yOffset += 10;

    doc.setFontSize(12);
    doc.text(`Total Marks: ${totalMarks}`, 10, yOffset);
    yOffset += 10;

    userAnswer.forEach((data, index) => {
      doc.text(`Question ${index + 1}: ${data.question}`, 10, yOffset);
      yOffset += 8;
      doc.text(`Given Answer: ${data.yourAnswer}`, 10, yOffset);
      yOffset += 8;
      doc.text(`Correct Answer: ${data.correctAnswer || "Not provided"}`, 10, yOffset);
      yOffset += 8;
      doc.text(`Points: ${data.points}`, 10, yOffset);
      yOffset += 10;
    });

    doc.save(`${name}-${category}-results.pdf`); // Save the PDF with the given filename
  };
    ...
    <>
      ...
      <div className="container">
        <div className="fixed-header">
          ...
          <button className="btn btn-secondary mx-2" onClick={downloadPDF}>
            Download PDF
          </button>
        </div>
        <div className={`container ${loadingStatus && "overlay"}`}>
